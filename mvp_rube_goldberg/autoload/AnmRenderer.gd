...
func render_frame(anm_name: String, state_id: int, state_counter: int = 0,
	...
	if not section_a.is_empty():
		...
		var section_c: Array = anm_data.get("section_c", [])
		var num_b_frames = anm_data.get("section_b", []).size()
		var frame_count = num_b_frames - first_a_idx
		for i in range(section_c.size()):
			var entry = section_c[i]
			if entry.get("state_id") == state_id:
				var cur_first = entry.get("first_frame_id", 0)
				var next_first = num_b_frames
				if i + 1 < section_c.size():
					next_first = section_c[i + 1].get("first_frame_id", next_first)
				frame_count = next_first - cur_first
				break
		section_b_idx = first_a_idx + (state_counter % maxi(frame_count, 1))
